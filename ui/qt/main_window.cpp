...
const QPacketListRecord &MainWindow::physicalRows() const
{
    return packet_list_->physicalRows();
}

void MainWindow::getItem(const QVectorLabelText &vecLabelText, StringListLabelText &listLabelText) const
{
    for (const auto &v : vecLabelText)
    {
        StringListLabelText labelText;

        labelText.labelText = v.labelText;
        getItem(v.vecLabelText, labelText);
        listLabelText.vecLabelText.push_back(labelText);
    }
}

void MainWindow::getItem(const QVectorLabelText &vecLabelText, QStringListLabelText &listLabelText) const
{
    for (const auto &v : vecLabelText)
    {
        StringListLabelText labelText;
        labelText.labelText = v.labelText;
        getItem(v.vecLabelText, labelText);
        listLabelText.push_back(labelText);
    }
}

QStringListLabelText MainWindow::getItem(int i) const
{
    if (i > physicalRows().size() || i < 0)
    {
        return {};
    }

    cf_select_packet(CaptureFile::globalCapFile(), i);

    capture_file* cap_file = CaptureFile::globalCapFile();
    if (cap_file && cap_file->edt && cap_file->edt->tree)
    {
        auto vec = ProtoNode(cap_file->edt->tree).allLabelText();
        QStringListLabelText listLabelText;
        getItem(vec, listLabelText);
