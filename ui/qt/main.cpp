...
#if _WIN32
    #define name_pcap "Z:\\pcap\\1.pcap"
#else
    #define name_pcap "/home/gitlab.oz/video_pcap/2kameri.pcap"
#endif

void getItem(const QStringListLabelText &vecLabelText, QString symbol)
{
    for (const auto &v : vecLabelText)
    {
        qDebug() << symbol << v.labelText;
        getItem(v.vecLabelText, symbol + symbol);
    }
}
...
int main(int argc, char *qt_argv[])
{
    ...
    main_w->openCaptureFile("Z:\\pcap\\1.pcap");

    for (int i = 0; i < 1; i++)
    {
        auto item = main_w->getItem(i);
        getItem(item, "    ");
    }
