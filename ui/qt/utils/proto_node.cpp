...
void ProtoNode::allLabelText(VectorLabelText &vecLabelText, ChildIterator kids) const
{
    if (!node_) return;

    ChildIterator child = kids.element().children();
    while (child.element().isValid())
    {
        VectorLabelText vecChild;

        vecChild.labelText = child.element().labelText();
        allLabelText(vecChild, child);
        vecLabelText.vecLabelText.push_back(vecChild);

        child.next();
    }
}

QVector<VectorLabelText> ProtoNode::allLabelText() const
{
    QVector<VectorLabelText> vecParent;

    if (!node_) return vecParent;

    ChildIterator kids = children();
    while (kids.element().isValid())
    {
        VectorLabelText vecChild;
        vecChild.labelText = kids.element().labelText();

        allLabelText(vecChild, kids);
        vecParent.push_back(vecChild);
